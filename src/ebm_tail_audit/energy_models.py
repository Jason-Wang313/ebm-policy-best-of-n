...
def support_blind_energy(pool: ArrayDict, seed: int = 4) -> np.ndarray:
    """Score visible progress while ignoring support validity."""

    rng = np.random.default_rng(seed)
    return (
        -1.12 * np.asarray(pool["visible_score"], dtype=float)
        - 0.62 * np.asarray(pool["shortcut_score"], dtype=float)
        + 0.02 * np.asarray(pool["action_norm"], dtype=float)
        + rng.normal(0.0, 0.025, size=len(pool["utility"]))
    )


def noisy_energy(pool: ArrayDict, seed: int = 5) -> np.ndarray:
    """A high-variance scorer with the right average sign but a ragged tail."""

    rng = np.random.default_rng(seed)
    return good_tail_aligned_energy(pool, seed=seed, noise_scale=0.10) + rng.normal(
        0.0, 0.42, size=len(pool["utility"])
    )


def pilot_label_noise_energy(pool: ArrayDict, seed: int = 6, label_noise: float = 0.55) -> np.ndarray:
    """Stress a utility-labeled repair path with noisy pilot-label-like scores."""

    rng = np.random.default_rng(seed)
    noisy_utility = np.asarray(pool["utility"], dtype=float) + rng.normal(0.0, label_noise, size=len(pool["utility"]))
    return -noisy_utility + 0.20 * np.asarray(pool["support_penalty"], dtype=float)
