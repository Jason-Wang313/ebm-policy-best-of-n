"""Exact finite tail selection laws for score-maximizing selection.
...
def utility_tail_selection_finite(
    ...
    """Expected selected utility from a finite pool under tail selection.
...
def binary_success_tail_selection(
    scores: Iterable[float] | np.ndarray,
    success: Iterable[float] | np.ndarray,
    n_values: Iterable[int],
    tie_policy: str = "mean",
) -> dict[int, float]:
    """Exact finite tail selection success probability for binary utilities."""
    ...
    return utility_tail_selection_finite(scores, success_arr, n_values, tie_policy=tie_policy)


def monte_carlo_tail_selection(
    scores: Iterable[float] | np.ndarray,
    utilities: Iterable[float] | np.ndarray,
    n_values: Iterable[int],
    n_trials: int = 20_000,
    ...
    """Monte Carlo estimate and normal-approximation CI for tail selection utility."""
