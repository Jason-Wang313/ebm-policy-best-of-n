...
from ebm_tail_audit.energy_models import raw_miscalibrated_energy, value_shaped_energy
...
def test_calibration_and_value_shaping_improve_high_n_tail_utility():
    ...
    raw_u = utility_tail_selection_finite(score_from_energy(raw), pool["utility"], [64])[64]
    cal_u = utility_tail_selection_finite(score_from_energy(calibrated), pool["utility"], [64])[64]
    shaped_u = utility_tail_selection_finite(score_from_energy(shaped), pool["utility"], [64])[64]
